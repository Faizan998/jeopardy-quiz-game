...
async function getProduct(id: string): Promise<Product | null> {
  try {
    ...
    const res = await fetch(`${baseUrl}/api/admin/store/product/${id}`, {
      cache: 'no-store',
    });
      ...
      <div className="bg-white rounded-lg shadow-xl overflow-hidden w-full sm:w-11/12 md:w-8/12 lg:w-6/12 xl:w-5/12 p-6">
        ...
        <div className="mb-4">
          <p className="text-md text-gray-500">
            <strong>Created On:</strong> {new Date(product.createdAt).toLocaleDateString()}
          </p>
        </div>
        ...
        <div className="flex justify-between mt-4">
          ...
          <Link 
            href={`/admin-dashboard/product-List/${id}/update`}
